...
type RecordItem = { contentId: string; type: ContentType; status: ContentStatus; title: string; slug: string; description: string; summary: string; category: string; author: string; body: string; takeaways: string[]; imageUrl: string; seoTitle: string; seoDescription: string; revisionCount: number; updatedAt?: string | null };
type FormState = Omit<RecordItem, "contentId" | "revisionCount" | "updatedAt" | "takeaways"> & { contentId?: string; takeawaysText: string };

const emptyForm: FormState = { type: "blog", status: "draft", title: "", slug: "", description: "", summary: "", category: "", author: "Soma Digital Community", body: "", takeawaysText: "", imageUrl: "", seoTitle: "", seoDescription: "" };
...
export default function AdminSiteContentPage() {
  ...
  const openRecord = (record: RecordItem) => setForm({ ...record, takeawaysText: record.takeaways.join("\n") });
  ...
  const payload = useMemo(() => ({ ...form, takeaways: form.takeawaysText.split("\n").map((item) => item.trim()).filter(Boolean), sections: form.body.trim() ? [{ heading: form.type === "case_study" ? "Workflow" : "Overview", body: form.body.trim() }] : [], relatedLinks: [] }), [form]);
      ...
      <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
          ...
          {preview ? <Preview form={form} /> : <>
            ...
            <div className="mt-4 grid gap-4"><Field label="SEO description"><textarea value={form.description} onChange={(event) => update("description", event.target.value)} rows={3} className="cms-input resize-y" placeholder="One concise description for search and sharing." /></Field><Field label="Answer summary"><textarea value={form.summary} onChange={(event) => update("summary", event.target.value)} rows={3} className="cms-input resize-y" placeholder="Answer the page topic directly in plain language." /></Field><Field label="Article body"><textarea value={form.body} onChange={(event) => update("body", event.target.value)} rows={14} className="cms-input resize-y font-mono text-sm" placeholder="Write the main content here. Use short paragraphs and clear headings." /></Field><Field label="Key takeaways (one per line)"><textarea value={form.takeawaysText} onChange={(event) => update("takeawaysText", event.target.value)} rows={5} className="cms-input resize-y" placeholder="One useful takeaway per line" /></Field></div>
