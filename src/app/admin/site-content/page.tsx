      ...
      <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
          ...
          {preview ? <Preview form={form} /> : <>
            ...
            <div className="mt-4 grid gap-4"><Field label="SEO description" help="Used in search results and social previews. Summarize the page promise in about 150–160 characters; do not repeat the title or add claims not supported by the article."><textarea value={form.description} onChange={(event) => update("description", event.target.value)} rows={3} className="cms-input resize-y" placeholder="Example: A practical guide to building a repeatable content system for a growing business." /></Field><Field label="Answer summary" help="Shown directly below the public title and used as the page's answer-focused introduction. Give a clear, plain-language answer in one short paragraph, usually 40–100 words."><textarea value={form.summary} onChange={(event) => update("summary", event.target.value)} rows={3} className="cms-input resize-y" placeholder="Start with the direct answer, then explain why it matters." /></Field>
...
function Field({ label, help, children }: { label: string; help?: string; children: React.ReactNode }) { return <label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-white/45">{label}</span>{help ? <span className="mb-2 block text-xs leading-5 text-white/40">{help}</span> : null}{children}</label>; }
