  ...
  return (
    <AppLayout>
      <article className="mx-auto max-w-4xl space-y-8">
        ...
        <header className="space-y-5"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-300">{study.category}</p><h1 className="max-w-3xl text-4xl font-bold font-headline text-white sm:text-6xl">{study.title}</h1><p className="max-w-3xl text-lg leading-8 text-[#BFC6D4]">{study.summary}</p><div className="flex items-center gap-2 text-sm text-muted-foreground"><CalendarDays className="h-4 w-4" />Published {study.publishedAt} · Updated {study.updatedAt}</div>{study.image ? <Image src={study.image} alt={study.title} width={1200} height={630} sizes="(max-width: 896px) 100vw, 896px" className="mt-6 max-h-[460px] w-full rounded-2xl border border-white/10 object-cover" /> : null}</header>
        <GlassCard className="border-cyan-400/15 bg-cyan-400/[0.04] p-6"><h2 className="text-lg font-semibold text-white">Key takeaways</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-[#BFC6D4]">{study.takeaways.map((takeaway) => <li key={takeaway} className="flex gap-3"><span className="text-cyan-300">•</span><span>{takeaway}</span></li>)}</ul></GlassCard>
        <div className="space-y-8">{study.sections.map((section) => <section key={section.heading} className="space-y-3"><h2 className="text-2xl font-semibold text-white">{section.heading}</h2><p className="text-base leading-8 text-[#BFC6D4]">{section.body}</p></section>)}</div>
        <section className="border-t border-white/[0.08] pt-6"><h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-muted-foreground">Continue exploring</h2><div className="mt-4 flex flex-wrap gap-3">{study.relatedLinks.map((link) => <Link key={link.href} href={link.href} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white hover:border-cyan-300/40 hover:text-cyan-200">{link.label}<ArrowRight className="h-4 w-4" /></Link>)}</div></section>
      </article>
    </AppLayout>
  );
}
