  ...
  return (
    <AppLayout>
      <article className="mx-auto max-w-4xl space-y-8">
        ...
        <section className="border-t border-white/[0.08] pt-6"><h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-muted-foreground">Continue exploring</h2><div className="mt-4 flex flex-wrap gap-3">{[...article.relatedLinks, { label: "Home", href: "/" }, { label: "Get started", href: "/open" }].map((link) => <Link key={`${link.label}-${link.href}`} href={link.href} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white hover:border-cyan-300/40 hover:text-cyan-200">{link.label}<ArrowRight className="h-4 w-4" /></Link>)}</div></section>
      </article>
    </AppLayout>
  );
}
